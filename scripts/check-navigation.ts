...
assert.equal(pages.length, 24, 'İstenen 24 alt sayfa bulunmalı');
...
assert.deepEqual(navigation.find((group) => group.title === 'Üretim')?.pages.map((page) => page.title), ['Sipariş Kartları', 'Üretim Takibi', 'Tamamlanan Üretimler', 'Çöp Kutusu', 'Arşivlenen Siparişler']);
...
console.log('Menü kontrolü başarılı: 6 modül, 24 benzersiz alt sayfa ve Para Hareketleri grubu.');
