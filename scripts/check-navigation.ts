...
assert.equal(pages.length, 23, 'İstenen 23 alt sayfa bulunmalı');
...
assert.deepEqual(navigation.find((group) => group.title === 'Üretim')?.pages.map((page) => page.title), ['Sipariş Kartları', 'Üretim Takibi', 'Tamamlanan Üretimler', 'Arşivlenen Siparişler']);
...
console.log('Menü kontrolü başarılı: 6 modül, 23 benzersiz alt sayfa ve Para Hareketleri grubu.');
