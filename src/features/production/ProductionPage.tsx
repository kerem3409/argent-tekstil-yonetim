...
import { commonSizeDistribution, cuttingTotal, cutRows, processes, productionSizes, stageRemainingQuantity, subcontractRows, workflowStageAmount } from '../../domain/productionWorkflow';
...
function ProductionDetail({ production: p, contacts, done, print, back, transferred }: { production: ProductionRecord; contacts: Contact[]; done: () => void; print: (kind: 'cutting' | 'tracking') => void; back: () => void; transferred: () => void }) {
  ...
  return <><div className="ws-tabs"><button className="button ws-secondary" onClick={back}>Listeye Dön</button>{!!p.cuttingSheet.brandSections.length && <button className="button ws-secondary" onClick={() => print('cutting')}>Kesim Föyü Yazdır / A4</button>}{cut && <button className="button ws-secondary" onClick={() => print('tracking')}>Üretim Takip Föyü Yazdır</button>}</div>
    ...
    {commonSizeDistribution(p) && <Section title={`Ortak Beden Serisi / Pastal Dağılımı · ${p.sizeSeries}`}><p className="ws-hint">Bu dağılım Üretim Kartındaki tüm renklere ortak uygulanır.</p><Table headers={['Beden', 'Pastal / Seri Adedi']} rows={productionSizes(p).map((size) => [size, commonSizeDistribution(p)?.[size] ?? 0])} /></Section>}
    ...
    {cut && <><Section title="Kesim Sonuçları"><Table headers={['Marka', 'Renk', 'Top Sayısı', 'Kg', 'Kesimden Çıkan Adet']} rows={cutRows(p).map((r) => [r.brandName, r.color, r.rollCount ?? '—', r.kg ?? '—', r.quantity])} /></Section>
      {!!commonSizeDistribution(p) && <Section title={`Ortak Beden Serisi / Pastal Dağılımı · ${p.sizeSeries}`}><p className="ws-hint">Bu dağılım tüm renklere ortak uygulanır.</p><Table headers={['Beden', 'Pastal / Seri Adedi']} rows={productionSizes(p).map((size) => [size, commonSizeDistribution(p)?.[size] ?? 0])} /></Section>}
