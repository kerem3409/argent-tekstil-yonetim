...
import { cuttingTotal, cutRows, processes, productionSizes, stageRemainingQuantity, subcontractRows, workflowStageAmount } from '../../domain/productionWorkflow';
...
function ProductionDetail({ production: p, contacts, done, print, back, transferred }: { production: ProductionRecord; contacts: Contact[]; done: () => void; print: (kind: 'cutting' | 'tracking') => void; back: () => void; transferred: () => void }) {
  ...
  return <><div className="ws-tabs"><button className="button ws-secondary" onClick={back}>Listeye Dön</button>{!!p.cuttingSheet.brandSections.length && <button className="button ws-secondary" onClick={() => print('cutting')}>Kesim Föyü Yazdır / A4</button>}{cut && <button className="button ws-secondary" onClick={() => print('tracking')}>Üretim Takip Föyü Yazdır</button>}</div>
    ...
    {p.plannedSizeDistributions && <Section title="Planlanan Beden Dağılımı"><Table headers={['Renk', ...productionSizes(p), 'Toplam']} rows={p.plannedSizeDistributions.map((d) => [d.color, ...productionSizes(p).map((s) => d.sizes[s] ?? 0), Object.values(d.sizes).reduce((a, b) => a + b, 0)])} /></Section>}
    ...
    {cut && <><Section title="Kesim Sonuçları"><Table headers={['Marka', 'Renk', 'Top Sayısı', 'Kg', 'Kesimden Çıkan Adet']} rows={cutRows(p).map((r) => [r.brandName, r.color, r.rollCount ?? '—', r.kg ?? '—', r.quantity])} /></Section>
      {!!p.sizeDistributions.length && <Section title={`Beden Dağılımı · ${p.sizeSeries}`}><Table headers={['Marka / Renk', ...productionSizes(p), 'Toplam']} rows={cutRows(p).map((r) => { const d = p.sizeDistributions.find((d) => d.rowId === r.id); return [`${r.brandName} / ${r.color}`, ...productionSizes(p).map((s) => d?.sizes[s] ?? 0), d ? Object.values(d.sizes).reduce((a, b) => a + b, 0) : 0]; })} /></Section>}
