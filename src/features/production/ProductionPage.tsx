...
import { commonSizeDistribution, cuttingTotal, cutRows, processes, stageRemainingQuantity, subcontractRows, workflowStageAmount } from '../../domain/productionWorkflow';
  ...
  return <Page title={titles[view]} error={error} loading={(!resource.data || !orders.data || !companies.data) && !error} reload={() => { resource.reload(); orders.reload(); companies.reload(); }}>
    {view === 'new' ? companies.data && <NewProductionForm contacts={companies.data} saved={(p) => navigate(detailHref(p))} />
      ...
      : <><div className="ws-tabs"><Link className="button" to="/uretim/yeni">Yeni Üretim</Link><Field label="Üretim / Ürün Ara"><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} /></Field></div>
        <Table headers={view === 'completed' ? ['Üretim No', 'Sipariş No', 'Marka', 'Ürün', 'Kesimden Çıkan', 'Sağlam Ürün', 'Dikim Firması', 'Tamamlanma Tarihi', 'İşlem'] : ['Üretim No', 'Sipariş No', 'Marka', 'Ürün / Model', 'Kesim Adedi', 'Güncel Durum', 'Dikim Firması', 'Tarih', 'Termin Tarihi', 'İşlem']} rows={(resource.data ?? []).filter((p) => view === 'completed' ? p.status === 'Tamamlandı' : view === 'tracking' ? p.status !== 'Tamamlandı' : p.status !== 'Tamamlandı').filter((p) => `${p.productionNo} ${p.productName} ${p.brand ?? ''}`.toLocaleLowerCase('tr-TR').includes(search.toLocaleLowerCase('tr-TR'))).map((p) => view === 'completed'
          ...
          : [p.productionNo, orderNo(p.orderCardId), p.brand ?? p.cuttingSheet.brandSections[0]?.brandName ?? '—', p.productName, p.cuttingSheet.completedAt ? cuttingTotal(p) : 'Kesim bekliyor', p.status, p.sewingCompanyId ? name(p.sewingCompanyId) : '—', p.date, <DueDate value={orders.data?.find((o) => o.id === p.orderCardId || o.productionCardIds.includes(p.id))?.dueDate} />, <Link className="button ws-secondary" to={detailHref(p)}>Üretim Kartını Aç</Link>])} />
      </>}
  </Page>;
}
...
function ProductionDetail({ production: p, order, contacts, done, print, back, transferred }: { production: ProductionRecord; order?: ProductionOrderCard; contacts: Contact[]; done: () => void; print: (kind: 'cutting' | 'tracking') => void; back: () => void; transferred: () => void }) {
  ...
  const cut = !!p.cuttingSheet.completedAt; const closed = !!p.completion || !!p.stockTransfer;
  const finish = () => { setEditing(''); done(); };
  return <><div className="ws-tabs"><button className="button ws-secondary" onClick={back}>Listeye Dön</button><button className="button ws-secondary" onClick={() => print('cutting')}>Kesime Föy Hazırla</button>{cut && <button className="button ws-secondary" onClick={() => print('tracking')}>Üretim Takip Föyü Yazdır</button>}</div>
    <p className="production-progress">Yeni Üretim → Kesime Föy → Kesim Sonucu → Üretimde → Üretim Tamamlama → Stoğa Aktarım<br /><strong>Güncel durum: {p.status}</strong></p>
    <Section title={`${p.productionNo} · ${p.productName}`}><dl className="production-summary">{[['Tarih', p.date], ['Kumaş', p.fabricName || '—'], ['Gramaj', p.gsm || '—'], ['Beden Serisi', p.legacy?.missingSizes && !p.sizeDistributions.length ? 'Eski kayıtta belirtilmemiş' : p.sizeSeries], ['Kesimci / Atölye', name(p.cutterCompanyId)], ['Kesim Şekli', p.cuttingMode === 'Hedef Adet' ? `Hedef ${p.targetQuantity} adet` : p.cuttingMode], ['Toplam Kesim Adedi', cut ? cuttingTotal(p) : 'Bekleniyor']].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className="ws-hint">Termin Tarihi: <DueDate value={order?.dueDate} /></p><p className="ws-hint">{p.productInstructions}</p><p className="ws-hint">{p.note}</p></Section>
