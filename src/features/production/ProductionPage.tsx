...
import { productionDisplayName, commonSizeDistribution, cuttingTotal, cutRows, processes, stageRemainingQuantity, subcontractRows, workflowStageAmount } from '../../domain/productionWorkflow';
...
import { CuttingEditor, NewProductionForm } from './ProductionForms';
...
import { ProductionCardEditor } from './ProductionCardEditor';
...
export function ProductionPage({ view }: { view: ProductionView }) {
  ...
  if (p && (print === 'cutting' || print === 'tracking')) return <Page title={print === 'cutting' ? 'Kesime Föy / A4' : 'Üretim Takip Föyü / A4'} error={error} loading={!orders.data || !companies.data}><div className="production-screen-only ws-tabs"><button className="button" onClick={() => window.print()}>Yazdır / A4 Föy</button><button className="button ws-secondary" onClick={() => setParams({ id: p.id })}>Üretime Dön</button></div><ProductionPrint production={p} order={orders.data?.find((o) => o.id === p.orderCardId || o.productionCardIds.includes(p.id))} kind={print} companyName={name} /></Page>;
  ...
  return <Page title={titles[view]} error={error} loading={(!resource.data || !orders.data || !companies.data) && !error} reload={() => { resource.reload(); orders.reload(); companies.reload(); }}>
    {view === 'new' ? companies.data && <NewProductionForm contacts={companies.data} saved={(p) => navigate(detailHref(p))} />
      : p ? <ProductionDetail key={`${p.id}:${p.revision}`} production={p} order={orders.data?.find((o) => o.id === p.orderCardId || o.productionCardIds.includes(p.id))} contacts={companies.data ?? []} done={done} print={(kind) => setParams({ id: p.id, yazdir: kind })} back={() => setParams({})} transferred={() => navigate(`/uretim/tamamlanan?id=${encodeURIComponent(p.id)}`)} />
      : id ? <p role="alert" className="ws-error">Üretim bulunamadı. <button onClick={() => setParams({})}>Listeye dön</button></p>
      : view === 'subcontracts' ? <WorkflowSubcontracts records={(resource.data ?? []).filter((p) => !p.deleted && !p.archived)} contacts={companies.data ?? []} />
      : <><div className="ws-tabs"><Link className="button" to="/uretim/yeni">Yeni Üretim</Link><Field label="Üretim / Ürün Ara"><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} /></Field></div>
        <Table headers={view === 'completed' ? ['Üretim No', 'Üretim Adı', 'Sipariş No', 'Marka', 'Ürün', 'Kesimden Çıkan', 'Sağlam Ürün', 'Dikim Firması', 'Tamamlanma Tarihi', 'İşlem'] : ['Üretim No', 'Üretim Adı', 'Sipariş No', 'Marka', 'Ürün / Model', 'Kesim Adedi', 'Güncel Durum', 'Dikim Firması', 'Tarih', 'Termin Tarihi', 'İşlem']} rows={(resource.data ?? []).filter((p) => !p.deleted && !p.archived).filter((p) => view === 'completed' ? p.status === 'Tamamlandı' : view === 'tracking' ? p.status !== 'Tamamlandı' : p.status !== 'Tamamlandı').filter((p) => `${p.productionNo} ${p.productionName ?? ''} ${p.productName} ${p.brand ?? ''}`.toLocaleLowerCase('tr-TR').includes(search.toLocaleLowerCase('tr-TR'))).map((p) => view === 'completed'
          ? [p.productionNo, productionDisplayName(p), orderNo(p.orderCardId), p.brand ?? p.cuttingSheet.brandSections[0]?.brandName ?? '—', p.productName, cuttingTotal(p), p.completion?.good, p.sewingCompanyId ? name(p.sewingCompanyId) : '—', p.stockTransfer?.date, <Link className="button ws-secondary" to={detailHref(p)}>Geçmiş / Detay</Link>]
          : [p.productionNo, productionDisplayName(p), orderNo(p.orderCardId), p.brand ?? p.cuttingSheet.brandSections[0]?.brandName ?? '—', p.productName, p.cuttingSheet.completedAt ? cuttingTotal(p) : 'Kesim bekliyor', p.status, p.sewingCompanyId ? name(p.sewingCompanyId) : '—', p.date, <DueDate value={orders.data?.find((o) => o.id === p.orderCardId || o.productionCardIds.includes(p.id))?.dueDate} />, <Link className="button ws-secondary" to={detailHref(p)}>Üretim Kartını Aç</Link>])} />
      </>}
  </Page>;
}

function ProductionDetail({ production: p, order, contacts, done, print, back, transferred }: { production: ProductionRecord; order?: ProductionOrderCard; contacts: Contact[]; done: () => void; print: (kind: 'cutting' | 'tracking') => void; back: () => void; transferred: () => void }) {
  const [editing, setEditing] = useState('');
  const name = (id: string) => id ? companyName(contacts, id) : 'Kendi Atölyemiz';
  const cut = !!p.cuttingSheet.completedAt; const closed = !!p.completion || !!p.stockTransfer || !!p.archived || !!p.deleted;
  const finish = () => { setEditing(''); done(); };
  return <><div className="ws-tabs"><RecordActions kind="production" record={p} done={done} /><button className="button ws-secondary" onClick={back}>Listeye Dön</button><button className="button ws-secondary" onClick={() => print('cutting')}>Kesime Föy Hazırla</button>{cut && <button className="button ws-secondary" onClick={() => print('tracking')}>Üretim Takip Föyü Yazdır</button>}</div>
    <p className="production-progress">Yeni Üretim → Kesime Föy → Kesim Sonucu → Üretimde → Üretim Tamamlama → Stoğa Aktarım<br /><strong>Güncel durum: {p.deleted ? 'Çöp Kutusunda' : p.archived ? 'Arşivde' : p.status}</strong></p>
    <Section title={`${p.productionNo} · ${productionDisplayName(p)}`}><dl className="production-summary">{[['Üretim No', p.productionNo], ['Üretim Adı', productionDisplayName(p)], ['Tarih', p.date], ['Marka', p.brand ?? '—']].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><h3 className="production-reference-heading">Siparişten Gelen Ürün Bilgisi</h3><dl className="production-summary">{[['Ürün / Model', p.modelName ?? p.productName], ['Ürün Tanımı', order?.items.find((item) => item.id === p.orderItemId)?.productName ?? p.productName], ['Kumaş', p.fabricName || '—'], ['Gramaj', p.gsm || '—'], ['Beden Serisi', p.legacy?.missingSizes && !p.sizeDistributions.length ? 'Eski kayıtta belirtilmemiş' : p.sizeSeries], ['Kesimci / Atölye', name(p.cutterCompanyId)], ['Kesim Şekli', p.cuttingMode === 'Hedef Adet' ? `Hedef ${p.targetQuantity} adet` : p.cuttingMode], ['Toplam Kesim Adedi', cut ? cuttingTotal(p) : 'Bekleniyor']].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className="ws-hint">Termin Tarihi: <DueDate value={order?.dueDate} /></p><p className="ws-hint">{p.productInstructions}</p><p className="ws-hint">{p.note}</p></Section>
    {!closed && <button className="button ws-secondary" onClick={() => setEditing(editing === 'card' ? '' : 'card')}>Düzenle / Güncelle</button>}
    {!closed && editing === 'card' && <ProductionCardEditor production={p} contacts={contacts} done={finish} />}
    {commonSizeDistribution(p) && <Section title={`Ortak Beden Serisi / Pastal Dağılımı · ${p.sizeSeries}`}><p className="ws-hint">Bu dağılım Üretim Kartındaki tüm renklere ortak uygulanır.</p><CommonSizes production={p} /></Section>}
    ...
    {!cut && !closed && <><button className="button ws-secondary" onClick={() => setEditing(editing === 'cutting' ? '' : 'cutting')}>Kesim Sonucu Gir</button>{editing === 'cutting' && <CuttingEditor production={p} done={finish} />}</>}
    {cut && <><Section title="Kesim Sonuçları"><Table headers={['Marka', 'Renk', 'Top Sayısı', 'Kg', 'Kesimden Çıkan Adet']} rows={cutRows(p).map((r) => [r.brandName, r.color, r.rollCount ?? '—', r.kg ?? '—', r.quantity])} /></Section>
      {!closed && <div className="ws-tabs"><button className="button" onClick={() => setEditing(editing === 'stage' ? '' : 'stage')}>+ Üretim Aşaması Ekle</button><button className="button ws-secondary" onClick={() => setEditing(editing === 'complete' ? '' : 'complete')}>Üretimi Tamamla</button></div>}
    ...
    {p.completion && <Section title={p.stockTransfer ? 'Tamamlandı / Stoğa Aktarıldı' : 'Üretim Tamamlandı — Stoğa Aktarım Bekliyor'}><p>Sağlam ürün: {p.completion.good} · Fire / Hatalı: {p.completion.waste} · Tamamlama: {p.completion.date}</p><p className="ws-hint">{p.completion.note}</p>{!!p.completion.lines.length && <Table headers={['Marka / Renk', 'Beden', 'Sağlam', 'Fire / Hatalı']} rows={p.completion.lines.map((l) => { const row = cutRows(p).find((r) => r.id === l.rowId); return [`${row?.brandName} / ${row?.color}`, l.size || 'Belirtilmemiş', l.good, l.waste]; })} />}{!p.stockTransfer ? !p.deleted && !p.archived && <Action run={() => workflowRepository.transfer(p.id)} done={transferred}>Stoğa Aktar</Action> : <><p className="ws-hint">Stoğa aktarım: {p.stockTransfer.date}. Kaynak: Üretim.</p><div className="ws-tabs">{p.stockTransfer.stockIds.map((id, i) => <Link className="button ws-secondary" key={id} to={`/stok/urunler?islem=gecmis&id=${id}`}>Stok Kaydı {i + 1}</Link>)}</div></>}</Section>}
  </>;
}
