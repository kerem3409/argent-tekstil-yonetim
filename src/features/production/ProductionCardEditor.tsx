...
import { CompanySelect, Field, Form, Input, Section, text } from '../shared/WorkshopUI';
...
export function ProductionCardEditor({ production: p, contacts, done }: { production: ProductionRecord; contacts: Contact[]; done: () => void }) {
  ...
  return <Section title="Üretim Kartını Güncelle"><p className="ws-hint">Marka değişiyorsa yeni Üretim Kartı oluşturulmalıdır. Sipariş bağlantıları ve ayrılan renk/adetler korunur.</p>
    <Form label="Üretim Kartını Güncelle" onDone={done} validate={() => { try { if (Object.keys(sizes).length) validateCommonSizeDistribution(series, sizes); return []; } catch (e) { return [e instanceof Error ? e.message : 'Beden dağılımını kontrol edin.']; } }} onSubmit={(f) => workflowRepository.updateProduction(p.id, p.revision, {
      productionDueDate: p.workflowVersion === 2 ? text(f, 'productionDueDate') : undefined, productionName: text(f, 'productionName') || undefined, brand: p.brand, sizeSeries: series, sizeDistribution: sizes,
      ...
      <div className="ws-grid production-identity"><Field label="Üretim No"><input readOnly value={p.productionNo} /></Field><Field label="Üretim Adı"><input name="productionName" required={p.workflowVersion === 2} defaultValue={p.productionName ?? ''} maxLength={200} /></Field><Field label="Tarih"><input readOnly value={p.date} /></Field><Field label="Marka"><input readOnly value={p.brand ?? ''} /></Field></div>
      {p.workflowVersion === 2 && <Input label="Üretim Termin Tarihi" name="productionDueDate" type="date" value={p.productionDueDate} required />}<PlannedSizes series={series} value={sizes} onChange={setSizes} onSeriesChange={setSeries} locked={pastalLocked(p)} />
      <div className="ws-grid"><AssignmentFields contacts={contacts} production={p} /><Field label="Kumaş Özellikleri"><textarea readOnly={p.workflowVersion === 2} name="fabricProperties" defaultValue={p.fabricProperties} maxLength={2000} /></Field><Field label="Ürün Detayı / Talimat"><textarea readOnly={p.workflowVersion === 2} name="instructions" defaultValue={p.productInstructions} maxLength={2000} /></Field><Field label="Not"><textarea name="note" defaultValue={p.note} maxLength={2000} /></Field></div>
