...
const emptyItem = (): DraftItem => ({ draftKey: crypto.randomUUID(), productDefinitionId: '', modelName: '', colorQuantities: [{ color: '', quantity: 0 }], fabricName: '', gsm: '', fabricProperties: '', productDetails: '' });
...
function ItemEditor({ item, definitions, update, remove, reload, selectProduct }: { item: OrderItemInput; definitions: ProductDefinition[]; update: (item: OrderItemInput) => void; remove: () => void; reload: () => void; selectProduct: (id: string) => void }) {
  const [adding, setAdding] = useState(false);
  return <div className="production-brand-editor">
    ...
    <button type="button" className="button ws-secondary" onClick={() => setAdding(!adding)}>+ Yeni Ürün Ekle</button>
    ...
    <strong>Renk Dağılımı</strong>
    {item.colorQuantities.map((row, i) => <div className="production-color-editor" key={i}><Field label="Renk *"><input value={row.color} required onChange={(e) => update({ ...item, colorQuantities: item.colorQuantities.map((r, j) => i === j ? { ...r, color: e.target.value } : r) })} /></Field><Field label="Adet *"><input type="number" min={1} step={1} required value={row.quantity || ''} onChange={(e) => update({ ...item, colorQuantities: item.colorQuantities.map((r, j) => i === j ? { ...r, quantity: Number(e.target.value) } : r) })} /></Field>{item.colorQuantities.length > 1 && <button type="button" className="button ws-secondary" onClick={() => update({ ...item, colorQuantities: item.colorQuantities.filter((_, j) => i !== j) })}>Satırı Sil</button>}</div>)}
    ...
    <div className="ws-grid">{(['fabricName', 'gsm', 'fabricProperties', 'productDetails'] as const).map((key) => <Field key={key} label={{ fabricName: 'Kumaş Adı *', gsm: 'Gramaj *', fabricProperties: 'Kumaş Özellikleri', productDetails: 'Ürüne Ait Özellikler / Talimatlar' }[key]}>{key === 'fabricName' || key === 'gsm' ? <input value={item[key]} required maxLength={2000} onChange={(e) => update({ ...item, [key]: e.target.value })} /> : <textarea value={item[key]} maxLength={2000} rows={2} onChange={(e) => update({ ...item, [key]: e.target.value })} />}</Field>)}</div>
    <button type="button" className="button ws-secondary" onClick={remove}>Sipariş Kalemini Kaldır</button>
  </div>;
}
...
export function OrderCardForm({ initial: initialOrder, definitions, contacts, reloadDefinitions, reloadContacts, done, cancel }: { initial?: ProductionOrderCard; definitions: ProductDefinition[]; contacts: Contact[]; reloadDefinitions: () => void; reloadContacts: () => void; done: () => void; cancel: () => void }) {
  ...
  const [items, setItems] = useState<DraftItem[]>(() => initial ? structuredClone(initial.items).map((i) => ({ ...i, draftKey: i.id, modelName: i.modelName ?? i.productName })) : [emptyItem()]);
  ...
  return <Section title={initial ? 'Sipariş Kartını Güncelle' : 'Yeni Sipariş Kartı'}>
    ...
    {quick && <QuickCustomer done={(id) => { setCustomerId(id); setQuick(false); reloadContacts(); }} />}
    <Form label={initial ? 'Güncelle' : 'Kaydet'} validate={(form) => { form.set('customerId', customerId); return validateOrderForm(form, items); }} onDone={done} onSubmit={async (form) => {
      const input = { orderType, customerId: customerId || undefined, date: text(form, 'date'), note: text(form, 'note'), items: items.map(({ draftKey: _key, ...item }) => item) };
      ...
      <div className="ws-grid"><Field label="Sipariş Türü"><select name="orderType" value={orderType} disabled={!!initial} onChange={(e) => setOrderType(e.target.value as typeof orderType)}><option>Ön Sipariş</option><option>Stok İçin Üretim</option></select>{initial && <input type="hidden" name="orderType" value={orderType} />}</Field><Input label="Sipariş Tarihi *" name="date" type="date" value={initial?.date ?? today()} required />
        <Field label={orderType === 'Ön Sipariş' ? 'Müşteri *' : 'Müşteri'}><select name="customerId" required={orderType === 'Ön Sipariş'} value={customerId} onChange={(e) => setCustomerId(e.target.value)}><option value="">Müşteri seçin</option>{historicalCustomer && <option value={customerId} disabled>{contacts.find((c) => c.id === customerId)?.name ?? 'Eski müşteri'} (mevcut kayıt)</option>}{customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></Field>
        <button type="button" className="button ws-secondary" onClick={() => setQuick(!quick)}>+ Yeni Müşteri Ekle</button><Field label="Genel Not"><textarea name="note" defaultValue={initial?.note ?? ''} maxLength={2000} rows={2} /></Field></div>
      <Section title="Sipariş Kalemleri">{items.map((item) => <ItemEditor key={item.draftKey} item={item} definitions={definitions} reload={reloadDefinitions} selectProduct={(id) => setItems((prev) => prev.map((current) => current.draftKey === item.draftKey ? { ...current, productDefinitionId: id } : current))} update={(next) => setItems((prev) => prev.map((current) => current.draftKey === item.draftKey ? { ...current, ...next } : current))} remove={() => setItems((prev) => prev.filter((current) => current.draftKey !== item.draftKey))} />)}<button type="button" className="button ws-secondary" onClick={() => setItems((prev) => [...prev, emptyItem()])}>+ Ürün Ekle</button></Section>
      <button type="button" className="button ws-secondary" onClick={cancel}>Vazgeç</button>
    </Form>
  </Section>;
}
