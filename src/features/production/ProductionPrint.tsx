...
import { commonSizeDistribution, cuttingTotal, productionSizes, stageRemainingQuantity } from '../../domain/productionWorkflow';
...
export function ProductionPrint({ production: p, kind, companyName }: { production: ProductionRecord; kind: 'cutting' | 'tracking'; companyName: (id: string) => string }) {
  const commonSizes = commonSizeDistribution(p);
  return <article className="production-paper" aria-label={kind === 'cutting' ? 'Kesim Föyü' : 'Üretim Takip Föyü'}>
    ...
    {!!commonSizes && <><h2>Ortak Beden Serisi / Pastal Dağılımı</h2><p>Bu dağılım üretim kartındaki tüm renklere ortak uygulanır.</p><table className="production-paper-table"><thead><tr>{productionSizes(p).map((s) => <th key={s}>{s}</th>)}</tr></thead><tbody><tr>{productionSizes(p).map((s) => <td key={s}>{commonSizes[s] ?? 0}</td>)}</tr></tbody></table></>}
