...
import { defaultSizeDistribution, productionSizes, orderAllocation, validateCommonSizeDistribution } from '../../domain/productionWorkflow';
...
export function NewProductionForm({ contacts, saved, orderCardId, orderItem, records = [] }: { contacts: Contact[]; saved: (p: ProductionRecord) => void; orderCardId?: string; orderItem?: ProductionOrderItem; records?: ProductionRecord[] }) {
  ...
  const [series, setSeries] = useState<SizeSeries>('Yetişkin'); const [sizes, setSizes] = useState<CommonSizeDistribution>(() => defaultSizeDistribution('Yetişkin'));
  ...
  function validate(f: FormData) {
    const errors: string[] = []; if (!text(f, 'productionName')) errors.push('Üretim Adı'); if (!text(f, 'brand')) errors.push('Marka');
    if (orderItem) {
  ...
  return <Section title="Yeni Üretim Kaydı"><p className="ws-hint">Her Üretim Kartı tek markaya aittir. Sipariş kalemini farklı markalar için ayrı kartlara bölebilirsiniz.</p>
    <Form validate={validate} onDone={() => {}} onSubmit={async (f) => { const p = await workflowRepository.create({ productionName: text(f, 'productionName'), productDefinitionId: product, brand: text(f, 'brand'), orderCardId, orderItemId: orderItem?.id, selectedColorQuantities: selected, sizeDistribution: sizes, sewingCompanyId: text(f, 'sewingCompanyId'), embroideryCompanyId: text(f, 'embroideryCompanyId'), printingCompanyId: text(f, 'printingCompanyId'), ironingPackagingCompanyId: text(f, 'ironingPackagingCompanyId'), fabricId: fabric, fabricName: orderItem?.fabricName ?? text(f, 'fabricName'), gsm, fabricProperties: text(f, 'fabricProperties'), sizeSeries: series, cuttingMode: 'Kumaştan Çıktığı Kadar', targetQuantity: null, cutterCompanyId: text(f, 'cutterCompanyId'), date: text(f, 'date'), productInstructions: text(f, 'instructions'), note: text(f, 'note') }); saved(p); }} label="Üretim Kartını Oluştur">
      <div className="ws-grid production-identity"><Field label="Üretim No"><input readOnly value="Otomatik" /></Field><Input label="Üretim Adı *" name="productionName" required /><Input label="Tarih *" name="date" type="date" value={today()} required /><Field label="Marka *"><input name="brand" required maxLength={200} /></Field></div><div className="ws-grid"><h3 className="production-reference-heading">Siparişten Gelen Ürün Bilgisi</h3>{orderItem ? <><Field label="Ürün Tanımı"><input readOnly value={orderItem.productName} /></Field><Field label="Ürün / Model"><input readOnly value={orderItem.modelName ?? orderItem.productName} /></Field></> : <ProductDefinitionSelect value={product} onChange={setProduct} quickAdd />}
...
export function CuttingEditor({ production: p, done }: { production: ProductionRecord; done: () => void }) {
  const [sections, setSections] = useState<BrandSection[]>(() => structuredClone(p.cuttingSheet.brandSections));
  function updateRow(brandId: string, rowId: string, changes: Partial<BrandSection['rows'][number]>) {
    setSections((prev) => prev.map((b) => b.id === brandId ? { ...b, rows: b.rows.map((r) => r.id === rowId ? { ...r, ...changes } : r) } : b));
  }
  if (!sections.some((b) => b.rows.length)) return <Section title="Kesim Sonucu"><p>Bu eski kartta kesime ayrılmış renk bulunmuyor. Kayıt korunuyor; yeni bir Sipariş Kaleminden Üretim Kartı oluşturabilirsiniz.</p></Section>;
  return <Section title="Kesim Sonucu Gir">
    <p className="ws-hint">Üretim kartının marka, renk ve ortak pastal talimatı kullanılır. Yalnızca kesimcinin bildirdiği sonuçları girin. Sonuç kaydedildikten sonra pastal kilitlenir.</p>
    <Form onDone={done} cancel={done} label="Kesim Sonucunu Kaydet" onSubmit={() => workflowRepository.saveCutting(p.id, p.revision, sections, true)}>
      {sections.map((b) => <div className="production-brand-editor" key={b.id}><h3>{b.brandName}</h3>
        {b.rows.map((r) => <div className="production-cut-result" key={r.id}>
          <Field label="Renk"><input readOnly value={r.color} /></Field>
      ...
      <p className="ws-hint">Toplam kesim adedi: {sections.reduce((sum, b) => sum + b.rows.reduce((n, r) => n + (r.quantity ?? 0), 0), 0)}</p>
    </Form>
  </Section>;
}
