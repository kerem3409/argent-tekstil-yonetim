...
export function Form({ children, onSubmit, onDone, label = 'Kaydet', validate, cancel, leadingAction }: { children: ReactNode; onSubmit: (form: FormData) => Promise<unknown>; onDone: () => void; label?: string; cancel?: () => void; leadingAction?: ReactNode; validate?: (form: FormData) => string[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (running.current) return; const form = new FormData(event.currentTarget); running.current = true; setBusy(true); setError(''); setValidationErrors([]); try { const errors = validate?.(form) ?? []; if (errors.length) { setValidationErrors(errors); return; } await onSubmit(form); onDone(); } catch (e) { setError(e instanceof Error ? e.message : 'İşlem başarısız.'); requestAnimationFrame(() => errorRef.current?.focus()); } finally { running.current = false; setBusy(false); } }
  return <form onSubmit={submit} noValidate={!!validate} className="ws-form">{error && <p className="ws-error" role="alert" tabIndex={-1} ref={errorRef}>{error}</p>}<fieldset disabled={busy}>{children}{validationErrors.length > 0 && <div className="ws-error" role="alert"><p>Lütfen aşağıdaki zorunlu alanları tamamlayın:</p><ul>{validationErrors.map((message) => <li key={message}>{message}</li>)}</ul></div>}<div className="ws-actions">{leadingAction}{cancel && <button type="button" className="button ws-secondary" onClick={cancel}>Vazgeç</button>}<button className="button">{busy ? 'Kaydediliyor…' : label}</button></div></fieldset></form>;
}
