...
export function Form({ children, onSubmit, onDone, label = 'Kaydet', validate }: { children: ReactNode; onSubmit: (form: FormData) => Promise<unknown>; onDone: () => void; label?: string; validate?: (form: FormData) => string[] }) {
  ...
  return <form onSubmit={submit} noValidate={!!validate} className="ws-form">{error && <p className="ws-error" role="alert" tabIndex={-1} ref={errorRef}>{error}</p>}<fieldset disabled={busy}>{children}{validationErrors.length > 0 && <div className="ws-error" role="alert"><p>Lütfen aşağıdaki zorunlu alanları tamamlayın:</p><ul>{validationErrors.map((message) => <li key={message}>{message}</li>)}</ul></div>}<div className="ws-actions"><button className="button">{busy ? 'Kaydediliyor…' : label}</button></div></fieldset></form>;
}
